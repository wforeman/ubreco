...
#include "NuSelectionBacktrackingFuncs.h"
#include "NuSelectionGeometry.h"
               ...
               const bool fLocaldEdx)
    {
      ...
      else
      {
        ...
        for (size_t i = 0; i < xyz_v.size(); i++)
        {
          auto xyz = xyz_v[i];
          float _dir[3];
          nuselection::TrkDirectionAtXYZ(trk, xyz.X(), xyz.Y(), xyz.Z(), _dir);
          std::vector<float> corr_par_value = nuselection::polarAngles(_dir[0], _dir[1], _dir[2], 2, plane);
        ...
        for (size_t i = 0; i < tp_indices.size(); i++)
        {
          ...
          is_hit_montecarlo.push_back(nuselection::isHitBtMonteCarlo(tp_index, assocMCPart, fEnergyThresholdForMCHits));
        }
        // correct hits
